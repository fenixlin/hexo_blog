...
**Disjoint Set**
数组记录的树，记录父节点及子树元素个数。查找：递归查找出树根a，然后沿路将父节点全部置为a。合并：要合并的两个节点分别查找出树根a,b，然后元素少的树根a父节点置为b
...
**Red-Black**
将结点分为红黑两类，所有空结点视为黑的叶节点，要求红结点必须连着黑结点，所有根到叶的路径黑结点数必须相同，而且根是黑结点。
插入时，插入点设为红。若父结点和兄弟同红，把父结点和兄弟变黑，祖父变红，然后回溯考虑祖父，直到1.父节点和兄弟异色，类似AVL，插入点是外儿子就做单旋/内儿子双旋，旋上去的结点为黑，另外两结点为红；2.父结点为黑；3.或已经到了根节点直接变为黑。
...
**SBT**
**B(B-), B+, B* Tree**
B(B-)：M叉树，根有2~M个儿子，其他内点cell(M/2)~M个儿子，叶结点同高，遵循BST左小右大的原则，一个结点(cell(M/2)-1)~(M-1)个值升序排列，每个值作为子树的值的分隔界，每个值在树中只出现一次。插入时一个结点放不下了，就分裂，中间作为分隔界的值放到父亲里面，如此递归。删除时将要删的值和左子树最大值(在叶结点)交换。叶结点删了后如果左右兄弟的值有多，其最大/最小值上移分隔界拿下来，否则合并兄弟删除分隔界再递归此问题到父结点。
B+：【待补充】
B\*：【待补充】

因为叉数比较多，深度浅，适用于访问结点代价昂贵的场合，如硬盘/数据库的读写。可能有不少空结点，比较浪费空间(除非牺牲效率，每层只顺序查找不用二分查找，就可以把每层弄成链表)。相比较而言，B树在记录每个子树大小后能快速访问第i大的点，而B+树适合连续访问一片数据。
**Trie**
每个点表示一个字母，或者二进制里面一位。根为空，从根走到某结点表示一个单词或数字，并在末尾结点存其相应属性的树。数据稠密，长度不大或可以并行处理的情况下可能比Hash表表现的好而且不用考虑冲突或hash法。不过比Hash表更费空间。
**Suffix Tree**
- 朴素：先在最后加一个字符串中没有的字符，然后对字符串的每个后缀串建立Trie，这样所有后缀都在叶节点上。然后对所有单儿子的路径压缩成一点。
- Ukkeonen：【待补充】
- 倍增：【待补充】
