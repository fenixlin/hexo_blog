...
*   状态压缩：将本来需要多维表达的状态，压缩为一个某进制（一般就是二进制、三进制）的整数来表达，整数的每一位就是原来一个维度。这样好处在于思路清晰代码简洁，二进制还可以方便地通过为运算进行状态转移。常见于矩阵、图上的动态规划。
...
被提到过但是觉得使用频率并不高的算法，此处暂不再进行学习总结，包括：
Rabin-Karp, Finger Tree,
