...
插入时，若父结点和兄弟同红，把父结点和兄弟变黑，祖父变红，然后回溯考虑祖父，直到1.父节点和兄弟异色，类似AVL做单旋/双旋后旋上去的结点为黑，另外两结点为红；2.父结点为黑；3.或已经到了根节点应变为黑。
删除时，做完BST的交换和删除后，若被删结点与其儿子N一红一黑，把儿子涂黑放上来就好。如果都是黑，主要考虑五个点(N的兄弟S及其内外两个儿子)。若S及儿子全黑，变为P黑S红。若仅S内侧儿子(RL/LR)红，内侧儿子旋上来变黑S变红变最后一种情况。其他情况(S红/外侧或双儿子红)把S单旋上来，SP换色其他红变黑。
写起来略复杂，不过平衡度要求没有AVL严格。所以插入和删除相比更快，而查找更慢。
**Trie**
